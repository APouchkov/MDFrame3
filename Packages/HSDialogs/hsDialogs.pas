...
procedure HSShowWarning(const Msg: string;
                        AEditorControl : TControl = nil;
                        AXCursorOffset : Integer = 0;
                        AYCursorOffset : Integer = 0);
...
var
  ...
  ButtonCaptions: array[TMsgDlgBtn] of Pointer = (
    ...
    @SHSMsgDlgNoToAll, @SHSMsgDlgYesToAll, @SHSMsgDlgHelp, @SHSMsgDlgClose);
...
begin
  ...
  if (AXCursorOffset = 0) and (AYCursorOffset = 0) and (AEditorControl <> nil) then begin
    AXCursorOffset := AEditorControl.Width shr 1;
    AYCursorOffset := -(AEditorControl.Height shr 1);
  end;
...
{ -=MD=- }
  ...
  if AEditorControl.Parent <> nil then
  ...
  if (_Left < WorkArea.Left)
      or (_Top < WorkArea.Top)
      or (_Left + Width > WorkArea.Right)
      or (_Top + Height > WorkArea.Bottom)
  then begin
    TrianglePosition := tpNone;
    Position := poMainFormCenter;
  end else begin
...
begin
  HSMessageDlg(Msg, mtError, [mbOK], 0, AEditorControl,
               AXCursorOffset, AYCursorOffset);
  //-- Соковнин А. В.: добавляем остановку всех действий
  Abort;
end;
