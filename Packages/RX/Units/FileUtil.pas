...
procedure CopyFileEx(const FileName, DestName: string; OverwriteReadOnly, ShellDialog: Boolean; ProgressControl: TControl);
...
uses {$IFDEF RX_D3} ActiveX, ComObj, ShlObj, {$ELSE} Ole2,
  OleAuto, {$ENDIF} {DateUtil,} ShellAPI, FileCtrl, Forms{, VCLUtils}
//  , RxPrgrss
  ;
...
function ResStr(Ident: string): string;
begin
  Result := Ident;
end;
...
begin
  if (Message.wParam = HelpButtonId) and ({$IFDEF WIN64}Int64Rec{$ELSE}LongRec{$ENDIF}(Message.lParam).Hi =
...
{$ENDIF}
  ...
  if (Length(Dir) < 3) or {$IFDEF DELPHIXE2}System.SysUtils.{$ENDIF}DirectoryExists(Dir) or
    (ExtractFilePath(Dir) = Dir) then Exit;
  ForceDirectories(ExtractFilePath(Dir));
  CreateDir(Dir);
end;

procedure CopyMoveFileShell(const FileName, DestName: string; Confirmation, AllowUndo, MoveFile: Boolean);
begin
